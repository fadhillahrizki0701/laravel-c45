...
class TestController extends Controller
{
    public function index()
    {
        ...
        $total_sangat_kurang = $this->getCount('berat_badan_per_usia', 'Sangat Kurang');

        $total_tbu_normal = $this->getCount('tinggi_badan_per_usia', 'Normal');
        $total_tbu_normal_gizi_baik = $this->getCount('tinggi_badan_per_usia', 'Normal', 'berat_badan_per_tinggi_badan', 'Gizi Baik');
    ...
    private function getCount(string $field, string $attr, string $field2 = '', string $attr2 = '')
    {
    ...
    private function getEntropy(string $attr = '')
    {
        ...
        $gizi_baik = $this->getCount('berat_badan_per_usia', 'Normal', 'berat_badan_per_tinggi_badan', 'Gizi Baik');
        $gizi_kurang = $this->getCount('berat_badan_per_usia', 'Normal', 'berat_badan_per_tinggi_badan', 'Gizi Kurang');

        if ($jumlah == 0 || $gizi_baik == 0 || $gizi_kurang == 0) {
            return 0;
        }

        $nilai = (-$gizi_baik / $jumlah) * log($gizi_baik / $jumlah, 2) + (-$gizi_kurang / $jumlah) * log($gizi_kurang / $jumlah, 2);
        return $nilai;
